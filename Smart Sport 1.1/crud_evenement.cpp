...
void crud_evenement::on_tabevent_activated(const QModelIndex &index)
{
    QString nom=ui->tabevent->model()->data(index).toString();
    QSqlQuery qry;

    qry.prepare("select * from evenement where nom = '"+nom+"' or ref ='"+nom+"'");
    if (qry.exec())
    {
        while(qry.next())
        {
...
void crud_evenement::on_pushButton_3_clicked()
{
    QString link="https://www.facebook.com/CL-Team-100727104987408/";
