...
void Don::on_lineEdit_id_6_textChanged(const QString &arg1)
{
     ...
     ui->data->setModel(tmp.displayClause("WHERE "+ui->comboBox->currentText()+" LIKE '%"+ui->lineEdit_id_6->text()+"%' ORDER BY "+ui->ID_DONATEUR_2->currentText()));
     else
...
void Don::on_lineEdit_id_7_textChanged(const QString &arg1)
{
    ...
    ui->data_2->setModel(tmp.displayClause("WHERE "+ui->comboBox_2->currentText()+" LIKE '%"+ui->lineEdit_id_7->text()+"%' ORDER BY "+ui->ID_DONATEUR_4->currentText()));
    else
    ui->data_2->setModel(tmp.afficher());
}
...
void Don::on_pushButton_clicked()
{  hide();
    menu c;
    c.exec();
...
void Don::on_pushButton_2_clicked()
{
    QString strStream;
                QTextStream out(&strStream);

                const int rowCount = ui->data->model()->rowCount();
                const int columnCount = ui->data->model()->columnCount();
                ...
                out <<"<html>\n"
                    ...
                    "<h1 style=\"text-align: center;\"><strong> ******Liste des Dons ****** </strong></h1>"
                    "<table style=\"text-align: center; font-size: 20px;\" border=1>\n "
                      "</br> </br>";
                // headers
                out << "<thead><tr bgcolor=#d6e5ff>";
                for (int column = 0; column < columnCount; column++)
                    if (!ui->data->isColumnHidden(column))
                        out << QString("<th>%1</th>").arg(ui->data->model()->headerData(column, Qt::Horizontal).toString());
                out << "</tr></thead>\n";
                ...
                for (int row = 0; row < rowCount; row++) {
                    ...
                    for (int column = 0; column < columnCount; column++) {
                        if (!ui->data->isColumnHidden(column)) {
                            QString data =ui->data->model()->data(ui->data->model()->index(row, column)).toString().simplified();
...
void Don::on_pushButton_3_clicked()
{
    QString strStream;
                QTextStream out(&strStream);

                const int rowCount = ui->data_2->model()->rowCount();
                const int columnCount = ui->data_2->model()->columnCount();
                QString TT = QDate::currentDate().toString("yyyy/MM/dd");
                out <<"<html>\n"
                      "<head>\n"
                       "<meta Content=\"Text/html; charset=Windows-1251\">\n"
                ...
                out << "<thead><tr bgcolor=#d6e5ff>";
                for (int column = 0; column < columnCount; column++)
                    if (!ui->data_2->isColumnHidden(column))
                        out << QString("<th>%1</th>").arg(ui->data_2->model()->headerData(column, Qt::Horizontal).toString());
                out << "</tr></thead>\n";
                ...
                for (int row = 0; row < rowCount; row++) {
                    ...
                    for (int column = 0; column < columnCount; column++) {
                        if (!ui->data_2->isColumnHidden(column)) {
                            QString data =ui->data_2->model()->data(ui->data_2->model()->index(row, column)).toString().simplified();
                            out << QString("<td bkcolor=0>%1</td>").arg((!data.isEmpty()) ? data : QString("&nbsp;"));
                        }
                    }
                    out << "</tr>\n";
