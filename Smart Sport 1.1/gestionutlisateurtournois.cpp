    ...
    ui(new Ui::GestionUtlisateurTournois)
{
    ...
    connect(ui->Email_Utilisateur, SIGNAL(clicked()),this, SLOT(sendMail()));
}
...
void GestionUtlisateurTournois::on_TabUtilisatuer_clicked(const QModelIndex &index)
{
    ...
    ui->Email->setText(ui->TabUtilisatuer->model()->data(ui->TabUtilisatuer->model()->index(ui->TabUtilisatuer->currentIndex().row(),2)).toString());
    ui->Motdepasse->setText(ui->TabUtilisatuer->model()->data(ui->TabUtilisatuer->model()->index(ui->TabUtilisatuer->currentIndex().row(),3)).toString());
    ui->Adresse->setText(ui->TabUtilisatuer->model()->data(ui->TabUtilisatuer->model()->index(ui->TabUtilisatuer->currentIndex().row(),4)).toString());
    ui->comboBox->setCurrentText(ui->TabUtilisatuer->model()->data(ui->TabUtilisatuer->model()->index(ui->TabUtilisatuer->currentIndex().row(),5)).toString());
}
...
void GestionUtlisateurTournois::on_Ajouter_Utilisateur_clicked()
{
  ...
  Utilisateurs u(Cin,Nom,Email,MotDePasse,Adresse,Role);
...
void GestionUtlisateurTournois::on_Modifier_Utilisateur_clicked()
{
    ...
    {
         Utilisateurs tmp(ui->TabUtilisatuer->model()->data(ui->TabUtilisatuer->model()->index(ui->TabUtilisatuer->currentIndex().row(),0)).toInt(),ui->Nom->text(),ui->Email->text(),ui->Motdepasse->text(),ui->Adresse->text(),ui->comboBox->currentText());
...
void GestionUtlisateurTournois::on_Imprimer_Utilisateur_clicked()
{
    QPrinter printer;
            ...
            ui->TabUtilisatuer->render(&printer);
}

void GestionUtlisateurTournois::on_Imprimer_Tournois_clicked()
{
    QPrinter printer;
            ...
            QPrintDialog dialog(&printer,this);
            if(dialog.exec()==QDialog::Rejected)return;
            ui->TabTournois->render(&printer);
}

void GestionUtlisateurTournois::on_Email_Utilisateur_clicked()
{
    QString MotDePasse= ui->Motdepasse->text();
    QString Email= ui->Email->text();

    Smtp* smtp = new Smtp("mtf244@gmail.com", "firastouihri24194179", "smtp.gmail.com",465);
           connect(smtp, SIGNAL(status(QString)), this, SLOT(mailSent(QString)));
           smtp->sendMail("mtf244@gmail.com",""+Email+"","Bienvenu", "Bienvenu dans notre assotiation voila votre mot de passe "+MotDePasse);
}
